...
#define CACHE_LINE_BYTES 512;
#define NUM_CACHE_LINES 0; // TODO add
...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    for (unsigned test=0; test < num_tests; test++) {
      ...
      auto input  = afu.malloc<dma_data_t>(CACHE_LINE_BYTES*NUM_CACHE_LINES);
      auto output  = afu.malloc<dma_data_t>(CACHE_LINE_BYTES*NUM_CACHE_LINES);  

      cout << "Starting Test " << test << "...";

      // Initialize the input and output memory.
      for (unsigned i=0; i < CACHE_LINE_BYTES*NUM_CACHE_LINES; i++) {
	...
	output[i] = 0;
     ...
     // unsigned total_bytes = size*sizeof(dma_data_t);
      ...
      afu.write(MMIO_SIZE, NUM_CACHE_LINES);
